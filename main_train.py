...
def main():
    sys.stdout = open('TPM_train_logs.txt', 'w')
    ...
    wandb.init(
        ...
        name='TPM_train_%dfd_%dsd' % (args.fold, args.seed),
...
def train(train_loader, model, criterion, optimizer, scheduler, args):
    ...
    for i, sample in enumerate(train_loader):
        ...
        query_pred, align_loss, thresh_loss = model(
            support_images, support_fg_mask, query_images,train=True, i=i, qry_mask=query_labels)  # FewShotSeg()
        ...
        if args.ADNet|args.adnet:
            loss = query_loss + align_loss + thresh_loss # thresh_loss : T loss
        else:
