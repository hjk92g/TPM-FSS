...
class TestDataset(Dataset):
    ...
    def getSupport(self, label=None, all_slices=True, N=None):
        ...
        if all_slices:
            ...
            sample['sprvxl'] = spr[:,idx][:,idx_]

            sample['image_all'] =torch.from_numpy(img)
            sample['sprvxl_all'] = spr
            if len(true_indices[idx_])==1:
                sample['idx'] = int(true_indices[idx_])  #Index according to all slice of support
            else:
                sample['idxs'] = [int(true_indices[idx_][i]) for i in range(len(true_indices[idx_]))]
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        self.min_size = 200
        self.non_succesive = args.non_succesive
