"""
Code from ALPNet (Ouyang et al.) (used in the 2D setting)
"""
...
class RandomAffine(object):
    ...
    Comment by ******: this is still doing affine on 2d (H x W plane).
    ...
    def build_M(self, input_shape):
        ...
        return final_tfx.astype(np.float32), params

    def __call__(self, image):
        # build matrix
        input_shape = image.shape[:2]
        M, params = self.build_M(input_shape)

        pre_mask_cnt=np.sum(image[:,:,3:])

        if (pre_mask_cnt == 0):
            print('    No foreground!(image_transform, RandomAffine)')
            print('    Pre RandA1:',pre_mask_cnt)

        res = np.zeros_like(image)
        if type(self.order) is list or type(self.order) is tuple:
            for i, intp in enumerate(self.order):
                res[..., i] = affine_transform_via_M(image[..., i], M[:2], interp=intp, params=params)
        else:
            # squeeze if needed
            orig_shape = image.shape
            image_s = np.squeeze(image)
            res = affine_transform_via_M(image_s, M[:2], interp=self.order, params=params)
            res = res.reshape(orig_shape)

        if (np.sum(res[:,:,3:]) == 0):
            print('    No foreground!(image_transform, RandomAffine)')
            print('    Pre RandA:', pre_mask_cnt)
            print('    Post RandA:', np.sum(res[:,:,3:]))
            plt.imshow(np.sum(image[:, :, 3:],axis=-1))
            plt.savefig('pre_zero_label_RA.png', dpi=200)
            plt.close()

            plt.imshow(np.sum(res[:, :, 3:],axis=-1))
            plt.savefig('post_zero_label_RA.png', dpi=200)
            plt.close()

        return res

def affine_transform_via_M(image, M, borderMode=cv2.BORDER_CONSTANT, interp=cv2.INTER_NEAREST, params=None):
    imshape = image.shape#(256, 256, 4)
    shape_size = imshape[:2]

    pre_mask_cnt = np.sum(image[:, :, 3:])
    if (pre_mask_cnt == 0):
        print('    No foreground!(image_transform, affine_transform_via_M)')
        print('    params:', params)
        print('    Pre affine_t_M:', pre_mask_cnt)
    ...
    if (np.sum(warped[:, :, 3:]) == 0):
        print('    No foreground!(image_transform, affine_transform_via_M)')
        print('    params:', params)
        print('    Pre affine_t_M:', pre_mask_cnt)
        print('    Post affine_t_M:', np.sum(warped[:, :, 3:]))
        plt.imshow(np.sum(image[:, :, 3:],axis=-1))
        plt.savefig('pre_zero_label_ATM.png',dpi=200)
        plt.close()

        plt.imshow(np.sum(warped[:, :, 3:],axis=-1))
        plt.savefig('post_zero_label_ATM.png', dpi=200)
        plt.close()
...
class ElasticTransform(object):
    ...
    def __call__(self, image):
        ...
        if (pre_mask_cnt == 0):
            print('    No foreground!(image_transform, ElasticTransform)')
            print('    Pre ElastT1:', pre_mask_cnt)
        ...
        if isinstance(self.sigma, Sequence):
            ...
            sigma = self.sigma
        result = elastic_transform_nd(image, alpha=alpha, sigma=sigma, order=self.order)
        if (np.sum(result[:,:,3:]) == 0):
            print('    No foreground!(image_transform, ElasticTransform)')
            print('    Pre ElastT:', pre_mask_cnt)
            print('    Post ElastT:', np.sum(result[:,:,3:]))
            plt.imshow(np.sum(image[:, :, 3:],axis=-1))
            plt.savefig('pre_zero_label_ET.png', dpi=200)
            plt.close()

            plt.imshow(np.sum(result[:, :, 3:],axis=-1))
            plt.savefig('post_zero_label_ET.png', dpi=200)
            plt.close()
        return result
